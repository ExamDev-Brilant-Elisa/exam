...
class Connexion():
    ...
    def listen(self, carteReseauEcoute, port, fichier):
        ...
        while True:
            ...
            threading.Thread(target=self.receive, args= (carteReseauConn ,fichier)).start()
            time.sleep(2)

    

    def receive(self, carteReseauConn, fichier):
        try:
            if len(listeIP) != 0:
                    i = 0
                    carteReseauConn.connect((listeIP[i], 60000))
                    print("Connection établie avec l'esclave")
                    i+=1
                    objetChoixAction.choix(carteReseauConn)
            else:
...
class ChoixAction(Connexion):
    ...
    def choix(self, carteReseauConn):
...
#va nous permettre de re-utiliser la carte réseau écoute
carteReseauEcoute.setsockopt(modSocket.SOL_SOCKET, modSocket.SO_REUSEADDR, 1)
